...
function getSkillCategories(data) {
  if (data.skillCategories && data.skillCategories.length > 0) {
    return data.skillCategories.filter(
  ...
  switch (key) {
    case "summary":
      return personalInfo.summary ? (
        <TemplateSection.Summary key={key} personalInfo={personalInfo} />
      ) : null;
    case "experience":
      return experience.some((exp) => exp.company || exp.position) ? (
        <TemplateSection.Experience key={key} experience={experience} />
      ) : null;
    case "education":
      return education.some((edu) => edu.institution || edu.degree) ? (
        <TemplateSection.Education key={key} education={education} />
      ) : null;
    case "skills":
      return skillCategories.length > 0 ? (
        <TemplateSection.Skills key={key} skillCategories={skillCategories} />
      ) : null;
    case "projects":
      return projects.some((proj) => proj.name) ? (
        <TemplateSection.Projects key={key} projects={projects} />
      ) : null;
    default:
      return null;
  }
...
function MinimalistTemplate({ data, sectionOrder }) {
  ...
  return (
    <div
      className="p-8 font-[var(--font-merriweather)] text-slate-800 text-sm leading-relaxed"
    ...
    >
      ...
      {sectionOrder.map((sectionKey) =>
        renderSectionByKey(sectionKey, data, MinimalistSections),
      )}
    </div>
  );
}
  ...
  return (
    <div className="font-sans text-slate-800 text-sm">
      ...
      <div className="p-6">
        {/* Render sections in order */}
        {sectionOrder.map((sectionKey) =>
          renderSectionByKey(sectionKey, data, ModernSections),
        )}
      </div>
    </div>
  );
    ...
    >
      ...
      <div className="px-6 py-4">
        <div className="border-t border-slate-200 pt-4">
          {/* Render sections in order */}
          {sectionOrder.map((sectionKey) =>
            renderSectionByKey(sectionKey, data, ExecutiveSections),
          )}
        </div>
      </div>
    </div>
...
export default function ResumePreview({ onOpenTemplate }) {
  ...
  const renderTemplate = () => {
    switch (activeTemplate) {
      case "minimalist":
        return (
          <MinimalistTemplate data={resumeData} sectionOrder={sectionOrder} />
        );
      case "modern":
        return <ModernTemplate data={resumeData} sectionOrder={sectionOrder} />;
      case "executive":
        return (
          <ExecutiveTemplate data={resumeData} sectionOrder={sectionOrder} />
        );
      default:
        return (
          <MinimalistTemplate data={resumeData} sectionOrder={sectionOrder} />
  ...
  return (
    <>
      ...
      <div id="print-container" style={{ display: "none" }}>
        <div className="a4-paper bg-white">{renderTemplate()}</div>
      </div>

      {/* Zoom Controls — compact bottom bar */}
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-1 bg-white/95 backdrop-blur-sm rounded shadow-md border border-slate-200 px-2 py-1 no-print">
        <button
          onClick={zoomOut}
          disabled={zoom <= 0.3}
          className={`p-1.5 rounded transition-colors ${zoom <= 0.3 ? "text-slate-300 cursor-not-allowed" : "text-slate-600 hover:bg-slate-100"}`}
          title="Zoom out"
        >
          <ZoomOut className="w-3.5 h-3.5" />
        </button>
        <span className="px-1.5 text-xs font-semibold text-slate-500 min-w-[40px] text-center tabular-nums">
          {Math.round(zoom * 100)}%
        </span>
        <button
          onClick={zoomIn}
          disabled={zoom >= 1.2}
          className={`p-1.5 rounded transition-colors ${zoom >= 1.2 ? "text-slate-300 cursor-not-allowed" : "text-slate-600 hover:bg-slate-100"}`}
          title="Zoom in"
        >
      ...
      {/* Preview canvas */}
      <div className="h-full overflow-auto p-6 pt-4 flex justify-center no-print relative">
        <div
          id="resume-preview"
          className="bg-white shadow-lg overflow-hidden border border-slate-300"
          style={{
            width: "210mm",
            minHeight: "297mm",
